...
<script type="text/javascript" src="<?php echo $this->webroot; ?>common/js/list.js"></script>
		...
		<ul id="topicpath">
			...
			<li><a href="<?php echo $this->webroot; ?>office/station/">事務所を探す 駅から探す</a></li>
			<li><?php echo h($station['Station']['name']); ?>の物件一覧</li>
		</ul>
		...
		<!-- search_list/ -->
		<h3 class="search_list_ttl"><?php echo h($station['Station']['name']); ?>駅周辺 の物件一覧</h3>

		<!-- search_list_map_nav/ -->
		<div id="map_canvas" class="search_list_map"></div>
		<?php echo $this->Form->hidden('lat', array('id'=>'map_lat', 'value'=>$station['Station']['lat'])); ?>
		<?php echo $this->Form->hidden('lng', array('id'=>'map_lng', 'value'=>$station['Station']['lng'])); ?>
		...
		<?php echo $this->Form->hidden('detail_url', array('id'=>'detail_url', 'value'=>$this->webroot . 'office/station/detail/')); ?>
...
<?php if (!empty($station['Station']['map'])) { ?>
		<!-- station_mapdetail/ -->
		<div class="station_mapdetail">
			<h4><img src="<?php echo $this->webroot; ?>common/images/search/station_mapdetail_btn01_off.png" width="730" height="50" alt="駅周辺マップ" /></h4>
			<p><img src="<?php echo $this->webroot; ?>upload/Stations/<?php echo h($station['Station']['map']); ?>" width="710" height="380" alt="駅周辺マップ" /></p>
		</div>
		<!-- /station_mapdetail -->
<?php } ?>

		<?php echo $this->Form->create('OfficeBuilding', array('url' => '/office/station/list/' . $station['Station']['id'], 'id' => 'U_OfficeBuildingStationListForm')); ?>
			<!-- search_nav/ -->
...
<?php
    $this->Paginator->options(array(
        'url' => array(
            'controller' => 'office',
            'action'     => 'station/list/' . $station['Station']['id'],
        )
    ));
?>
...
<?php foreach ($officeBuildings as $officeBuilding) { ?>
		...
		<div class="search_column_ttl clearfix">
			<h3><a href="<?php echo $this->webroot; ?>office/station/detail/<?php echo h($officeBuilding['OfficeBuilding']['id']); ?>"><?php echo h($officeBuilding['OfficeBuilding']['name']); ?></a></h3>
			<p>
...
<?php     if (isset($officeBuilding['OfficePhoto']['0']['path']) && !empty($officeBuilding['OfficePhoto']['0']['path'])) { ?>
...
<?php     if (isset($officeBuilding['OfficePhoto']['1']['path']) && !empty($officeBuilding['OfficePhoto']['1']['path'])) { ?>
				<div>
					<p>
						<span><img src="<?php echo $this->webroot; ?>common/images/spacer.gif" alt="" /></span>
			...
			<div class="search_column_section">
				...
				<col width="">
					...
					<tr>
						...
						<td><?php echo h($areas[$officeBuilding['OfficeBuilding']['area_id']]); ?></td>
					</tr>
					<tr>
...
<?php     } ?>
				...
				<p class="imgbtn"><a href="<?php echo $this->webroot; ?>office/station/detail/<?php echo h($officeBuilding['OfficeBuilding']['id']); ?>"><img src="<?php echo $this->webroot; ?>common/images/search/search_column_btn01.png" width="180" height="30" alt="物件の詳細を見る" /></a></p>
			</div>
		</div>
<?php } ?>
...
<?php echo $this->element('paging'); ?>
		...
		<?php echo $this->Form->create('OfficeBuilding', array('url' => '/office/station/list/' . $station['Station']['id'], 'id' => 'B_OfficeBuildingStationListForm')); ?>
			<!-- search_nav/ -->
			<div class="search_nav clearfix">
				<h4 class="imgbtn"><img src="<?php echo $this->webroot; ?>common/images/search/search_nav_btn01.png" width="230" height="48" alt="条件を絞り込んで検索" /></h4>
				<div class="search_nav_detail">
