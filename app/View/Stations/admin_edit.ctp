...
<div class="row-fluid">
	...
	<ul class="nav nav-tabs">
		...
		<li class="active"><a><?php __h('駅の編集'); ?></a></li>
	</ul>
...
<?php } ?>
				...
				<tr>
					...
					<td><?php echo $this->Form->select('line_id', array($lines)); ?>
...
<?php } ?>
				...
				<tr>
					...
					<td>
<?php if (!empty($this->data['Station']['map'])) { ?>
					<p><?php echo $this->Html->link('Image', Configure::read('UploadDir') . $this->name . '/' . $this->data['Station']['map'], array('target' => '_blank')); ?></p>
<?php } else { ?>
					<p>No Image</p>
<?php }  ?>
					<?php echo $this->Form->file('new_map'); ?>
					<?php echo $this->Form->hidden('map', array()); ?>
